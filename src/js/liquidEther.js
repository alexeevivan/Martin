...
export function createLiquidEther(container, options = {}) {
	...
	class WebGLManager {
		...
		init() {
			console.log('WebGLManager: init() called, wrapper:', this.props.$wrapper);
			console.log('WebGLManager: canvas element:', Common.renderer.domElement);
			this.props.$wrapper.prepend(Common.renderer.domElement);
			console.log('WebGLManager: canvas added to DOM');
	...
	// Initialize
	// Don't override position if already set
	if (!container.style.position) {
		container.style.position = 'absolute';
	}
	container.style.overflow = container.style.overflow || 'hidden';

	console.log('LiquidEther: Initializing with container:', container);
	...
	const sim = webglManager.output?.simulation;
	if (sim) {
	...
	resizeObserver = new ResizeObserver(() => {
		...
		resizeRafId = requestAnimationFrame(() => {
			if (webglManager) webglManager.resize();
		});
	});
