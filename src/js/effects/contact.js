...
export function createColorBends(containerSelector, options = {}) {
	const {
		rotation = 90,
		speed = 0.2,
		colors = ['#ff6c6c', '#6c9fff', '#ff6cfd'],
		transparent = true,
		autoRotate = 0,
		scale = 1,
		frequency = 1,
	...
	for (let i = 0; i < MAX_COLORS; i++) {
		if (i < colorVectors.length) {
			uColorsArray[i].copy(colorVectors[i]);
		}
	}
	...
	const loop = () => {
		...
		const deg = (rotationValue % 360) + autoRotate * elapsed;
