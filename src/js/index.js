...
const slides = [
	{
		title: "nav-home",
		media: slideVideo,      // показывается если isVideo не работает — fallback
		isVideo: true,
		mediaVideo: slideVideo,  // основной источник для VideoTexture
		duration: 12000  // 12 секунд вместо стандартных 5000
	},
	{
		title: "nav-intro",
		media: remoteSlide
...
const navigateToSlide = (targetIndex) => {
	...
	if (introOverlay) {
		if (currentSlideIndex === 1 && targetIndex !== 1) {
			...
			}, 400); // совпадает с длительностью transition
		} else {
			introOverlay.classList.toggle('visible', targetIndex === 1);
		}
	}

	// Показываем/скрываем плеер
...
// Загрузка видео-текстуры с оптимизацией
const loadVideoTexture = (src) => {
	return new Promise((resolve, reject) => {
		...
		video.preload = 'metadata'; // Загружаем только метаданные, не весь файл
		...
		const onMetadata = () => {
			...
			texture.userData = {
				...
				video: video // Сохраняем ссылку для cleanup
			};

			// Запускаем воспроизведение сразу после получения метаданных
...
// Универсальная функция — сама определяет тип
...
window.addEventListener("load", async () => {
	...
	// Initialize Liquid Ether background for player slide
	const liquidEtherContainer = document.getElementById('liquid-ether-bg');
	console.log('Looking for liquid-ether-bg container:', liquidEtherContainer);
	if (liquidEtherContainer) {
		console.log('Container found, initializing liquidEther...');
...
document.addEventListener("visibilitychange", () => {
	if (document.hidden) {
		...
		// Resume current video when tab is visible
		const currentTexture = slideTextures[currentSlideIndex];
		if (currentTexture?.userData.video) {
